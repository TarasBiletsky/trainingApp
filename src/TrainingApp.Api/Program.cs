...
api.MapPost("/auth/register", async (LoginRequest request, bool? web, AppDbContext db, HttpContext http, CancellationToken ct) =>
{
    if (await db.Users.AnyAsync(x => x.UserName == request.UserName, ct)) return Results.Conflict(new { error = "Username already exists" });
    var user = new LocalUser { UserName = request.UserName, PasswordHash = "pending" };
    user.PasswordHash = new PasswordHasher<LocalUser>().HashPassword(user, request.Password);
    db.Add(user);
...
api.MapPost("/auth/logout", async (RefreshRequest? request, AppDbContext db, HttpContext http, CancellationToken ct) => { if (request?.RefreshToken is { Length: > 0 }) { var hash = TokenFactory.Hash(request.RefreshToken); var token = await db.RefreshTokens.SingleOrDefaultAsync(x => x.TokenHash == hash, ct); if (token != null) token.RevokedAt = DateTimeOffset.UtcNow; await db.SaveChangesAsync(ct); } await http.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme); return Results.NoContent(); }).Produces(204).Produces(401).RequireAuthorization();

var users = api.MapGroup("/users").RequireAuthorization();
users.MapGet("/me", async (HttpContext h, AppDbContext db, CancellationToken ct) => { var id = UserId(h); var user = await db.Users.AsNoTracking().SingleAsync(x => x.Id == id, ct); return Results.Ok(new UserResponse(user.Id, user.UserName, user.IsAdmin, user.IsActive, user.CreatedAt)); });
...
users.MapGet("/", async (AppDbContext db, CancellationToken ct) => await db.Users.AsNoTracking().OrderBy(x => x.UserName).Select(x => new UserResponse(x.Id, x.UserName, x.IsAdmin, x.IsActive, x.CreatedAt)).ToListAsync(ct)).RequireAuthorization("admin");
users.MapPost("/", async (CreateUserRequest r, AppDbContext db, CancellationToken ct) => { if (string.IsNullOrWhiteSpace(r.UserName) || r.Password.Length < 12) return Results.BadRequest(new { error = "Username is required and password must contain at least 12 characters" }); if (await db.Users.AnyAsync(x => x.UserName == r.UserName.Trim(), ct)) return Results.Conflict(new { error = "Username already exists" }); var user = new LocalUser { UserName = r.UserName.Trim(), PasswordHash = "pending", IsAdmin = r.IsAdmin }; user.PasswordHash = new PasswordHasher<LocalUser>().HashPassword(user, r.Password); db.Add(user); await db.SaveChangesAsync(ct); return Results.Created($"/api/v1/users/{user.Id}", new UserResponse(user.Id, user.UserName, user.IsAdmin, user.IsActive, user.CreatedAt)); }).RequireAuthorization("admin");
users.MapPut("/{id:guid}", async (Guid id, UpdateUserRequest r, HttpContext h, AppDbContext db, CancellationToken ct) => { var user = await db.Users.FindAsync([id], ct); if (user is null) return Results.NotFound(); if (id == UserId(h) && r.IsActive == false) return Results.BadRequest(new { error = "An administrator cannot deactivate the current account" }); if (r.Password is { Length: > 0 and < 12 }) return Results.BadRequest(new { error = "Password must contain at least 12 characters" }); user.IsActive = r.IsActive; user.IsAdmin = r.IsAdmin; if (!string.IsNullOrEmpty(r.Password)) { user.PasswordHash = new PasswordHasher<LocalUser>().HashPassword(user, r.Password); var tokens = await db.RefreshTokens.Where(x => x.UserId == id && x.RevokedAt == null).ToListAsync(ct); tokens.ForEach(x => x.RevokedAt = DateTimeOffset.UtcNow); } await db.SaveChangesAsync(ct); return Results.Ok(new UserResponse(user.Id, user.UserName, user.IsAdmin, user.IsActive, user.CreatedAt)); }).RequireAuthorization("admin");
...
workouts.MapGet("/", async (DateTimeOffset? from, DateTimeOffset? to, HttpContext h, AppDbContext db, CancellationToken ct) => { var owner = UserId(h); return await db.Workouts.Where(x => x.OwnerId == owner && (!from.HasValue || x.ScheduledAt >= from) && (!to.HasValue || x.ScheduledAt < to)).OrderBy(x => x.ScheduledAt).AsNoTracking().ToListAsync(ct); }).Produces<IReadOnlyList<Workout>>().Produces(401);
...
workouts.MapPost("/{id:guid}/exercises", async (Guid id, WorkoutExerciseWrite r, HttpContext h, AppDbContext db, CancellationToken ct) => { var owner = UserId(h); if (!await db.Workouts.AnyAsync(x => x.Id == id && x.OwnerId == owner, ct) || !await db.Exercises.AnyAsync(x => x.Id == r.ExerciseId && (x.OwnerId == null || x.OwnerId == owner), ct)) return Results.NotFound(); var x = new WorkoutExercise { Id = r.Id ?? Guid.NewGuid(), WorkoutId = id, ExerciseId = r.ExerciseId, Order = r.Order, Notes = r.Notes, RestSeconds = r.RestSeconds }; db.Add(x); await db.SaveChangesAsync(ct); return Results.Created($"/api/v1/workouts/{id}", x); });
...
static SetEntry NewSet(Guid parent, SetWrite r) => new() { Id = r.Id ?? Guid.NewGuid(), WorkoutExerciseId = parent, Order = r.Order, PlannedWeightKg = r.PlannedWeightKg, PlannedReps = r.PlannedReps, PlannedRpe = r.PlannedRpe, ActualWeightKg = r.ActualWeightKg, ActualReps = r.ActualReps, ActualRpe = r.ActualRpe, IsWarmup = r.IsWarmup, Notes = r.Notes };
...
static async Task<IResult> UpdateSet(Guid workoutId, Guid setId, SetWrite? r, SetStatus? status, Guid owner, AppDbContext db, CancellationToken ct) { var x = await db.SetEntries.Include(x => x.WorkoutExercise).ThenInclude(x => x!.Workout).SingleOrDefaultAsync(x => x.Id == setId && x.WorkoutExercise!.WorkoutId == workoutId && x.WorkoutExercise.Workout!.OwnerId == owner, ct); if (x is null) return Results.NotFound(); if (r?.Version is long expected && expected != x.Version) return Results.Conflict(new { error = "Version conflict", current = x }); if (r != null) { x.Order = r.Order; x.PlannedWeightKg = r.PlannedWeightKg; x.PlannedReps = r.PlannedReps; x.PlannedRpe = r.PlannedRpe; x.ActualWeightKg = r.ActualWeightKg; x.ActualReps = r.ActualReps; x.ActualRpe = r.ActualRpe; x.IsWarmup = r.IsWarmup; x.Notes = r.Notes; } if (status.HasValue) { x.Status = status.Value; x.CompletedAt = status == SetStatus.Completed ? DateTimeOffset.UtcNow : null; } x.Version++; x.UpdatedAt = DateTimeOffset.UtcNow; await db.SaveChangesAsync(ct); return Results.Ok(x); }
...
static async Task<IResult> SyncWorkouts(IReadOnlyList<SetSyncCommand> commands, HttpContext h, AppDbContext db, CancellationToken ct) { var owner = UserId(h); if (commands.Count is < 1 or > 100) return Results.BadRequest(new { error = "Batch size must be 1..100" }); var results = new List<SyncResult>(); foreach (var c in commands) { if (!await db.WorkoutExercises.AnyAsync(x => x.Id == c.WorkoutExerciseId && x.Workout!.OwnerId == owner, ct)) { results.Add(new SyncResult(c.Id, "rejected")); continue; } var current = await db.SetEntries.Include(x => x.WorkoutExercise).ThenInclude(x => x!.Workout).SingleOrDefaultAsync(x => x.Id == c.Id && x.WorkoutExercise!.Workout!.OwnerId == owner, ct); if (current is null) { var created = NewSet(c.WorkoutExerciseId, c.Value with { Id = c.Id }); db.Add(created); results.Add(new SyncResult(created.Id, "created", created.Version)); continue; } if (current.Version != c.ExpectedVersion) { results.Add(new SyncResult(current.Id, "conflict", current.Version, current)); continue; } current.ActualWeightKg = c.Value.ActualWeightKg; current.ActualReps = c.Value.ActualReps; current.ActualRpe = c.Value.ActualRpe; current.Status = c.Value.Status ?? current.Status; current.Version++; current.UpdatedAt = DateTimeOffset.UtcNow; results.Add(new SyncResult(current.Id, "updated", current.Version)); } await db.SaveChangesAsync(ct); return results.Any(x => x.Status == "conflict") ? Results.Conflict(results) : Results.Ok(results); }
...
public record CreateUserRequest(string UserName, string Password, bool IsAdmin = false); public record UpdateUserRequest(bool IsActive, bool IsAdmin, string? Password); public record ChangePasswordRequest(string CurrentPassword, string NewPassword); public record UserResponse(Guid Id, string UserName, bool IsAdmin, bool IsActive, DateTimeOffset CreatedAt);
...
public record WorkoutExerciseWrite(Guid? Id, Guid ExerciseId, int Order, string? Notes, int RestSeconds = 90);
public record SetWrite(Guid? Id, int Order, decimal? PlannedWeightKg, int? PlannedReps, decimal? PlannedRpe, decimal? ActualWeightKg, int? ActualReps, decimal? ActualRpe, bool IsWarmup, string? Notes, long? Version, SetStatus? Status = null);
public record SetSyncCommand(Guid Id, Guid WorkoutExerciseId, long ExpectedVersion, SetWrite Value);
public record TemplateWrite(Guid? Id, string Name, IReadOnlyList<TemplateExerciseWrite> Exercises); public record TemplateExerciseWrite(Guid? Id, Guid ExerciseId, int Order, int RestSeconds, IReadOnlyList<TemplateSetWrite> Sets); public record TemplateSetWrite(Guid? Id, int Order, decimal? WeightKg, int? Reps, decimal? Rpe, bool IsWarmup); public record TemplateWorkoutRequest(Guid? Id, string? Name, DateTimeOffset ScheduledAt);
